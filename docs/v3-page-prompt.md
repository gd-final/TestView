# TradeHub Supply V3 — 서브 페이지 제작 프롬프트

이 프롬프트는 `main_v3.html / main_v3.css / main_v3.js` 를 디자인 기준으로 삼아
다른 서브 페이지를 일관성 있게 제작할 때 그대로 복사해서 사용한다.

---

## 사용 방법

아래 전체 내용을 Claude에게 붙여넣고, 마지막에 만들 페이지명만 바꿔서 요청한다.
...
아래 기준을 **절대적으로** 따라서 TradeHub Supply BtoB 쇼핑몰의 서브 페이지를 구현해라.

---

## [프로젝트 개요]

- 서비스명: TradeHub Supply
- 서비스 성격: BtoB 무역/배송 종합 쇼핑몰 (기업 고객 대상)
- 기술 스택: Spring Boot + Thymeleaf + JPA (서버사이드 렌더링)
- 디자인 기준: 기존 메인페이지 V3 시안 (`main_v3.html`)
...
모든 페이지의 최대 너비는 `1320px`, 좌우 패딩은 `24px`로 통일한다.
...
<header class="gh">
  <div class="gh__wrap">
    <!-- 로고 (SVG 마크 + 워드마크) -->
    <a href="/" th:href="@{/}" class="gh__brand" aria-label="TradeHub Supply 홈">
      <svg class="gh__brand-icon" width="28" height="28" viewBox="0 0 28 28" fill="none">
      ...
      <span class="gh__brand-name">TradeHub<strong>Supply</strong></span>
    </a>
    ...
    <form class="gh__search" action="/search" th:action="@{/search}" method="get" role="search">
      <div class="gh__search-box">
        ...
        <input type="search" name="q" class="gh__search-input"
               placeholder="품목명, 카테고리, 품번으로 검색" aria-label="상품 검색" autocomplete="off">
        <div class="gh__search-divider"></div>
        <select name="category" class="gh__search-select" aria-label="카테고리 선택">
          ...
          <option value="package">포장/배송</option>
          <option value="promo">판촉물</option>
          <option value="season">시즌상품</option>
        </select>
      </div>
    </form>
    ...
    <nav class="gh__util" aria-label="유틸 메뉴">
      <a href="/login"       th:href="@{/login}"       class="gh__util-btn"><i class="fas fa-user"></i><span>로그인</span></a>
      <a href="/mypage"      th:href="@{/mypage}"      class="gh__util-btn"><i class="fas fa-th-large"></i><span>마이페이지</span></a>
      <a href="/order/track" th:href="@{/order/track}" class="gh__util-btn"><i class="fas fa-shipping-fast"></i><span>배송조회</span></a>
...
.badge--blue   { background: #dbeafe; color: #1d4ed8; }   /* 기업인기, 처리중 */
.badge--green  { background: #dcfce7; color: #15803d; }   /* 배송완료, 신상품 */
.badge--teal   { background: #ccfbf1; color: #0f766e; }   /* 당일출고 */
.badge--amber  { background: #fef3c7; color: #92400e; }   /* 대기중 */
.badge--red    { background: #fee2e2; color: #c0392b; }   /* 취소, 특가 */
.badge--gray   { background: #f1f5f9; color: #64748b; }   /* 기본/비활성 */
```
...
<head>
  ...
  <title>[페이지명] — TradeHub Supply</title>
...
1. 모든 색상은 `:root` 변수 참조. 절대 하드코딩 금지.
2. 클래스명은 **BEM 경량 방식** (블록__요소--변형자) 또는 기능명 단축형 사용.
3. 페이지 고유 클래스는 파일명 약어로 네임스페이스를 준다.
   - 예: 장바구니 → `.cart-*`, 고객센터 → `.cs-*`, 주문 → `.order-*`
4. 반응형은 태블릿 `768px`, 모바일 `480px` 두 단계만 처리한다.
5. 애니메이션은 `transition: var(--t)` 이상으로 복잡하게 만들지 않는다.
6. `box-shadow`는 `var(--s-xs)`, `var(--s-sm)`, `var(--s-md)` 세 단계만 사용한다.

---

## [JavaScript 작성 규칙]

V3 메인 페이지와 동일한 **순수 함수 + 공유 state 객체 패턴**을 사용한다.
...
- DOM 쿼리는 `init()` 내부 또는 함수 내 지역변수로 처리한다 (전역 남용 금지).
...
1. **헤더/푸터/GNB는 main_v3.html에서 복사**해서 사용하고, 구조를 바꾸지 않는다.
2. 페이지 고유 CSS만 별도 파일로 분리한다. (공통 CSS는 나중에 `v3-common.css`로 분리 예정)
3. **더미 데이터는 실제 서비스처럼 자연스럽게** 작성한다. (한국어, BtoB 문체)
4. Thymeleaf `th:each` 반복 구조는 반드시 적용해서 서버 연동이 쉽도록 작성한다.
5. **Empty state (빈 상태)**를 반드시 구현한다. `th:if="${#lists.isEmpty(list)}"` 활용.
6. **모바일 반응형**은 768px / 480px 기준 두 단계만 처리한다.
7. JS는 바닐라 JS만 사용하고, `state + render + bindEvents + init` 패턴을 따른다.
8. 접근성: `aria-label`, `role`, `aria-current`, `disabled` 속성을 적극 사용한다.
